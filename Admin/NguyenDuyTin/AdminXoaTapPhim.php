...
<body style="background-color: #d3cfcf;">
    ...
    $maPhim = $confirm = $maTapPhim = "";
    if (isset($_GET['maPhim']))
        $maPhim = $_GET['maPhim'];

    if (isset($_GET['maTapPhim']))
    ...
    if (isset($_GET['confirm'])) {
        ...
        if ($confirm == "co") {
            ...
            // lịch sử
            $sqlDel = "DELETE FROM `lichsu` WHERE `MaTapPhim`= '$maTapPhim' ";
            // kết quả của truy vấn 
            $resultDel = mysqli_query($conn, $sqlDel);
            //tập phim
            $sqlDel = "DELETE FROM `tapphim` WHERE `MaTapPhim`= '$maTapPhim' ";
            // kết quả của truy vấn 
            $resultDel = mysqli_query($conn, $sqlDel);
        }
        header("Location: AdminPhimInfo.php?maPhim=$maPhim");
    }
    ...
    // tập
    $sql4 = "SELECT * FROM `tapphim` WHERE `MaPhim`='$maPhim' AND `MaTapPhim`='$maTapPhim' ";
    // kết quả của truy vấn 
    ...
    <?php if ($numRows == 0 || $numRows4 == 0) echo "<div class='container-fluid' style='height:300px;'><h1> Xin lỗi, có thể không tìm thấy phim này</h1> </div>" ?>
    <div <?php
            if ($numRows == 0 || $numRows4 == 0) echo "hidden"; ?>>
        <!-- bảng tập  phim -->
        <div class="container" style="overflow-x: auto;">
            <table class="table table-striped table-bordered table-hover">
                <thead>
                    <tr>
                        <th colspan="3" class="text-center">Có chắc chắn muốn xóa tập phim này?<br>
                            <a href='<?php echo "AdminXoaTapPhim.php?maPhim=$maPhim&maTapPhim=$maTapPhim&confirm=co" ?>'> <button onclick="alert('Đã thực hiện xong thao tác')" class="btn btn-warning">Có</button> </a>
                            <a href='<?php echo "AdminXoaTapPhim.php?maPhim=$maPhim&maTapPhim=$maTapPhim&confirm=khong" ?>'> <button class="btn btn-warning">Không</button></a>
                        </th>
                    </tr>
                    <tr>
                        <th>
        ...
        <div class="container" style="overflow-x: auto;">
            <table class="table table-striped table-bordered table-hover">
                <tbody>
                    <tr>
                        <th colspan="4" class="text-center">Tập này thuộc về bộ phim:
